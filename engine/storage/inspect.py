...
def _meta_header_fields(raw: bytes, meta: MetaPage) -> tuple[HeaderField, ...]:
    layout: tuple[tuple[str, int, int, Any, str], ...] = (
        ...
        (
            ...
            "first heap page of chendb_indexes",
        ),
        ("catalog_indexes_last", 52, 4, meta.catalog_indexes_last, "its last page"),
        (
            "next_object_id",
            56,
            4,
            meta.next_object_id,
            "id the next table or index will get",
        ),
        ("lsn", 60, 8, meta.lsn, "reserved for the WAL (Milestone 9)"),
        ("flags", 68, 4, meta.flags, "reserved"),
        (
            "checksum",
            72,
            4,
            int.from_bytes(raw[72:76], "little"),
            "CRC32 over bytes 0..72",
        ),
    )
    return tuple(
        HeaderField(
...
def _page_header_fields(raw: bytes, page: Page) -> tuple[HeaderField, ...]:
    ...
    layout: tuple[tuple[str, int, int, Any, str], ...] = (
        ...
        (
            ...
            1,
            raw[12],
            "0 free, 1 meta, 2 heap, 3 schema, 4 btree-internal, 5 btree-leaf",
        ),
